...
//getPostTime的作用是将时间转换为几分钟前，几小时前，几天前，几月前，几年前
String getPostTime(String date){
  ...
  if(dt.year == DateTime.now().year && dt.month == DateTime.now().month && dt.day == DateTime.now().day){
    ...
    if(durs.day >= 1){
      ...
      return '${DateFormat.jm().format(dt)} - ${DateFormat("dd MMM yy").format(dt)}';
    }
  }
}
...
void share(String message,{required String subject}) {
