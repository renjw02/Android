...
class FeedsDetailScreen extends StatefulWidget {
  ...
  const FeedsDetailScreen(
      {super.key,
      required this.id,
      required this.post,
      required this.onRefreshBloc});
...
class _FeedsDetailScreenState extends State<FeedsDetailScreen>
  ...
  void initState() {
    super.initState();
    isLike = isLike = widget.post.supportList
        .map((dynamic) => dynamic.toString())
        .toList()
        .contains(CustomAuth.currentUser.uid);
  }
        ...
        body: AnimatedSize(
          ...
          child: isCommentPanelOpen
              ? CommentPanel(
                  onClose: _onScroll,
                  post: widget.post,
                  onRefreshBloc: widget.onRefreshBloc)
              : _buildBody(context),
        ),
                ...
                child: IconButton(
                  ...
                  icon: Stack(children: [
                    ...
                    Positioned(
                        left: 40.0 -
                            10.0 *
                                widget.post.supportList.length
                                    .toString()
                                    .length,
                        top: 2,
                        child: Row(children: [
  ...
  Widget _buildListBody(BuildContext context, Post item) {
    ...
    children
      ...
      ..add(item.images.length + item.videos.length == 0
          ? const SizedBox()
          : SizedBox(
    ...
    final commentsList = item.comments
        .map((kid) => Comment(comment: comment_model.Comment.fromDbMap(kid)))
        .toList();
    //将comment按照时间排序
...
class _UserProfileWidgetState extends State<UserProfileWidget> {
  ...
  initState() {
    super.initState();
    isFollowed = CustomAuth.currentUser.following
        .map((dynamic) => dynamic.toString())
        .toList()
        .contains(widget.creatorId);
    if (kDebugMode) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: 10),
          widget.creatorId != CustomAuth.currentUser.uid
              ? IconButton(
                  iconSize: 40,
                  // padding: EdgeInsets.all(10), // 内边距10
                  constraints: const BoxConstraints(
